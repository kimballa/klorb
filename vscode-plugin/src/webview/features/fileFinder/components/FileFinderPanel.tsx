...
import { splitFinderPath } from '../fileFinderModel';
 ...
 * splits its path into a truncatable directory part and a fixed file part (`splitFinderPath`) so
 * a deeply nested path reads as "some/path/to.../file.txt" instead of wrapping or scrolling
 * horizontally. Keeps the active row scrolled into view as `activeIndex` changes via keyboard
 * navigation.
 */
  ...
  return (
    <div className="file-finder-panel">
      {paths.map((path, index) => {
        const { dirPart, filePart } = splitFinderPath(path);
        ...
        return (
          <div
            ...
            <span className="file-finder-row-file">{filePart}</span>
          </div>
        );
      })}
    </div>
