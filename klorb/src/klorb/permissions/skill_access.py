...
class SkillRules(BaseModel):
    ...
    `format_fqsn`/`parse_fqsn`), not a nested array -- unambiguous since a skill name contains no
    path separator, and friendlier in a hand-edited config than a two-element array. Immutable
    after construction, like `CommandRules`."""
