...
* A literal token with no `*` in it must equal the candidate token at that exact position.
* A literal token that embeds a `*` inside it (e.g. `--arg=*`, `of=*`, `*.py`) is a
  single-token glob: it still occupies exactly one argv position (never zero, never two), but
  that position's candidate token only has to match the glob, not equal it verbatim -- each `*`
  inside the token matches any run of characters (including none) within that one token. Use
  this to generalize just the *value* half of a `--flag=value` or `key=value` style argument
  while keeping the flag/key name itself literal, instead of replacing the whole token with a
  bare `*` (which would also accept a completely different flag in that position).
...
input source literal -- prefer this single-token-glob form over `["dd", "if=/dev/zero", "*",
...
safe thing to generalize. This includes an embedded-glob argv0 like `["py*", "test"]` -- it still
means "any program whose name happens to match this glob," not a safe generalization. The one
...
glob. Anything else with a wildcard argv0 -- e.g. `["*", "-c", "*"]`, `["**", "status"]`,
`["py*", "test"]` -- is invalid and will be rejected; keep the program name literal instead. For
an item whose `kind` is not `"command"`
...
def _has_unsafe_wildcard_argv0(pattern: list[str]) -> bool:
    ...
    *is* what decides what happens. The one defensible exception is asking an arbitrary program for
    ...
    `["**", "status"]`, `["?", "run"]`, or an embedded-glob argv0 like `["py*", "test"]` (see
    `klorb.permissions.command_access._token_matches_literal`) — is unsafe, so a persistent grant
    is never built from it.
    """
    if not pattern:
        return False
    argv0 = pattern[0]
    if argv0 not in _WILDCARD_TOKENS and "*" not in argv0:
        return False
    return not (
