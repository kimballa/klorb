...
"""Computes and persists permission grants for the interactive "ask" confirmation flow:
what gets added to `readDirs`/`writeDirs.allow` or `.deny`, and removed from `.ask`,
in memory and on disk, when a user answers a `PermissionAskRequired`/
`MultiPermissionAskRequired` prompt with a persistent scope.
"""
...
"""Where a persistent decision is recorded."""
...
def compute_grant_paths(
    ...
    unioned with `write_dirs`, or, if
    nothing matched at all ("the dir was never mentioned"), `[candidate]` when `candidate` is
    itself a directory or `[candidate.parent]` when it's a file. This granularity computation
    is the same for an Allow or a Deny decision.
...
def _apply_decision_to_table(
    ...
    untouched: an "Allow, always" decision never strips an existing `deny` entry,
    and a "Deny, always" decision never strips an existing `allow`
    entry. Never mutates `rules` in place, per
...
def _load_file_dir_rules(path: Path) -> tuple[dict[str, Any], DirRules, DirRules]:
    """Read `path`'s own raw `sessionDefaults.readDirs`/`writeDirs`,
    returned as `(full_raw_contents, read_dirs, write_dirs)`.
    `full_raw_contents` is returned alongside so the caller can write it back with only
    `readDirs`/`writeDirs` replaced, preserving every other key in the file untouched.
    """
...
def _clean_ask_entries_only(path: Path, workspace_root: Path, granted_paths: list[Path]) -> None:
    ...
    the file back. A no-op if `path`
    doesn't exist, or if neither table has a matching `ask` entry.
    """
    if not path.is_file():
        return
    raw, file_read_dirs, file_write_dirs = _load_file_dir_rules(path)
...
def apply_permission_grant(
    ...
    """Record a permanent Allow or Deny decision for `path` at `scope`:
    ...
    - An `"allow"` decision always reassigns `session_config.read_dirs` (and `write_dirs`, when
      ...
      grant immediately; write access requires both tables to say `allow`,
      which is why a write grant always promotes read too. A `"deny"` decision only
      touches the one table matching
      `is_write` (`write_dirs` for a write ask, `read_dirs` for a read ask).
    ...
    - `"workspace"`/`"homedir"` additionally reassign `process_config.session`'s matching
      table(s) (so a future `/clear` in this process inherits the decision too)
      and persist it to the scope's target file. The file-persist step doesn't
