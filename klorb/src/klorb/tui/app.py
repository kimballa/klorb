...
the mixins in `klorb.tui.mixins`.
"""
...
class SelectionSafeScreen(Screen[None]):
    ...
    def action_copy_text(self) -> None:
        """Copy the current screen-level (mouse-drag) text selection to the clipboard.
        Also records the press for `ReplApp.action_interrupt`'s Ctrl+C streak bookkeeping
        (`ReplApp._note_ctrl_c_copy`). Raises `SkipAction` when nothing is selected, so the
        binding chain falls through to `ReplApp.action_interrupt`.
        """
...
class ReplApp(
    ...
    def __init__(
        ...
        """Set only by a real `klorb.cli.main()` invocation. `None` (the default, and every
        existing test's constructor call) disables workspace trust resolution, startup prompts,
        history announcement, and the palette command.
        """
        ...
        and exits the process instead of leaving the REPL open.
        """
        ...
        `klorb.tui.entrypoint.run_repl` prints this to stdout once `App.run()` returns and
        the TUI has actually torn down, so a quit-on-success exit still leaves the agent's
        final answer visible on the terminal.
        """
        self._file_index: WorkspaceFileIndex | None = None
        """The `@`-mention file finder's workspace index, started (`_start_file_finder_index`)
        only once workspace trust is resolved and only for a real `trust_manager`, so a `ReplApp`
        built without one never spawns a background filesystem watcher against a real directory.
        """
        ...
        """What the most recent Ctrl+C press (at `_last_ctrl_c_at`) actually did, or `None`
        ...
        turn (so repeated Escape/Ctrl+C don't stack duplicate notices).
        """
        ...
        actually taken hold (the turn has wound down).
        """
        ...
        snoozing it (`_snooze_watchdog`) for `watchdog.timeout` seconds. Started/snoozed in
        `on_mount`, disarmed in `on_unmount`.
        """
        ...
        serialize turns: pressing Enter posts a `Submitted` message synchronously, but `disabled`
        is set only later, when the app *handles* that message. Two submits queued inside that
        window (a double Enter, a bracketed paste containing a newline, ...) therefore both reach
        `on_prompt_input_submitted` before either disables the box, and without this flag both
        would dispatch concurrent `_send_prompt` workers. Being a plain bool touched only from
        the event-loop thread, its check-and-set is atomic against other message handlers.
        """
        ...
        `#interaction-panel` at a time. A single turn's tool calls are already serial, and
        ...
        showed, seeded into the next one's `initial_action`/`initial_scope` so answering several
        asks in a row for one compound tool call doesn't require re-navigating the grid to the
        same spot each time. Starts at the grid's default top-left cell; updated after every
        dismissal, including "Other..." (which reports back as its `action="deny"`/`scope="once"`
        ...
        session is selected (nothing created it, so there is no handle).
        """
        self._attention_needed: dict[str, None] = {}
        """Session ids with an ask waiting for the user to select them, in the order they
        started waiting. Drives the panel's blinking `(!)` marker and the status-line fallback
        (`SubagentsPanelMixin._update_subagent_attention_status_line`).
        """
        self._subagent_drafts: dict[str, str] = {}
        """Prompt-input draft text saved per session id, restored on reselecting a session that
        was blurred away from with unsubmitted text still in the box.
        """
        ...
        shows, letting `SubagentsPanelMixin._append_new_subagent_messages` mount only the delta on
        ...
        render, letting `_append_new_subagent_messages` notice a running/finished transition (and
        ...
        selected) but whose handle hasn't finished yet, or `None`. Keeps
        ...
        before `send_turn()` returns to `_send_prompt`, i.e. before `_finish_turn` ever runs.
        """
    ...
    def get_default_screen(self) -> Screen:
        ...
        crash the app via Textual's text-selection hit-test.
        """
    ...
    def workspace_files(self) -> list[str]:
        ...
        if `_file_index` was never started.
        """
    ...
    def select_model(self, name: str) -> None:
        ...
        future session started in this process, and the default for every future klorb process.
        """
    ...
    def _wire_session_notice_handler(self, session: Session) -> None:
        """Register this app as `session`'s `HookOutput.log` notice sink, posting a
        `TuiHistoryNotice` so a hook firing on any thread lands in the history scroll via
        `on_tui_history_notice`/`show_notice`.
        """
    ...
    def _wire_session_wake_handler(self, session: Session) -> None:
        """Register this app as `session`'s wake handler, posting a `TuiSessionWake`."""
        def wake() -> None:
    ...
    def show_notice(self, message: str, *, error: bool = False) -> None:
        ...
        item. Exposed for callers outside `ReplApp` that report a one-off status/result but
        shouldn't otherwise depend on history-widget internals like `HISTORY_ID`/`VerticalScroll`.
        Constructed with `markup=False`: `message` can be arbitrary, which must render verbatim
    ...
    def on_tui_history_notice(self, message: TuiHistoryNotice) -> None:
        ...
        conversation history via `show_notice()`.
        """
    ...
    def get_system_commands(self, screen: Screen) -> Iterable[SystemCommand]:
        """Every default system command except "Theme"."""
    ...
    def format_title(self, title: str, sub_title: str) -> Content:
        ...
        marker when the workspace isn't trusted, followed by the selected session's model and,
        if it has thinking enabled, its effort level in parentheses.
        """
    ...
    def _refresh_header_title(self) -> None:
        ...
        watch. `mutate_reactive` re-runs those watchers with `self.sub_title`'s value unchanged,
        which is enough to make `Header` re-invoke `format_title()`.
        """
    ...
    def set_thinking_enabled(self, enabled: bool) -> None:
        ...
        any future session started in this process, and the default for every future klorb
        process.
        """
