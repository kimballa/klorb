...
def build_server_parser() -> argparse.ArgumentParser:
    """Build the argument parser for `klorb server`'s own flags (`--config`).
    """
...
def run_server_cli(argv: list[str]) -> int:
    ...
    every other subcommand, so a malformed `--config` file is
    ...
    Installs no `SIGINT` handling of its own: a `SIGINT` is left to the
    ...
    as opaque text. Unlike a one-shot prompt, a server
