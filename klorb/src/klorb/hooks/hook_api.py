...
class HookOutput(BaseModel):
    ...
    `onSessionEnd`'s own result never does, since a session that's ending never has a live host
    to deliver a continuation to; `HookDispatcher` drops it from that hook's aggregate result,
    logged at `warning`. Valid only alongside a non-empty `message`; `HookDispatcher` also drops
    an aggregate result that sets this without one, logged at `warning`."""
    log: str | None = None
