...
"""`ServerStreams`/`AcpServer`: the Agent Client Protocol server behind `klorb server`."""
...
"""Buffer limit for the stdin `asyncio.StreamReader` `ServerStreams.from_stdio()` builds. ACP
frames one JSON-RPC message per line, and an image `session/prompt` request's `data` field is
the *raw* attachment, base64-inflated (~4/3) and client-side-capped at 25MB per image, with no
cap on attachment count per turn -- comfortably over the default limit, which fails closed as an
...
class AcpServer:
    ...
    `session/new` request's `SessionConfig` is copied from. `provider`/`model_registry`/
    `trust_manager`/`config_flag_path` are forwarded to the `KlorbAcpAgent` this constructs.
    A test harness injects a scripted `ApiProvider` and an isolated `TrustManager` here
    instead. The `KlorbAcpAgent` is built once, here, and exposed via `agent` so a test
    harness can inspect its live `Session` directly.
    """
    ...
    async def run(self) -> int:
        ...
        marked unstable in the ACP spec/SDK -- the SDK's own request router rejects it with
        `method_not_found` outright unless this flag is set, regardless of whether the agent
        implements the method. `session/load` and every other implemented method are unaffected
        either way.
        """
