...
"""Library logic behind `klorb init`. See docs/specs/klorb-init.md."""
...
"""`~`-relative directory `klorb init --user`'s symlink step creates `klorb` inside."""


class InitError(Exception):
    """Raised when a `klorb init` step can't complete. The message is safe
...
@dataclass
class StepResult:
    """The ordered progress messages produced by one `klorb init` step that actually changed
    or inspected disk state.
    """
...
def default_scope() -> InitScope:
    """`"system"` when running as root (effective uid 0), `"user"` otherwise.
    """
    return "system" if os.geteuid() == 0 else "user"


def config_target_path(scope: InitScope) -> Path:
    """Where `klorb init`'s config-file step writes for `scope`: `etc_config_path()` for
    `"system"`, `user_config_path()` for `"user"`.
    """
...
def is_user_scope_initialized() -> bool:
    ...
    `run_init("user", force=False)` is a pure no-op, so the TUI's "Init local klorb config"
    palette command hides itself rather than showing itself, so a broken symlink still counts
    as already there.
    """
...
def template_config_text() -> str:
    ...
    `importlib.resources`. Copied verbatim, not parsed
...
def copy_tiktoken_cache() -> StepResult:
    ...
    a later klorb process start can point tiktoken at it without needing network access.
    Unlike `write_config_file`/`create_symlink`, not scoped to `"system"`/`"user"`, so
    `run_init()` runs this step once regardless of `scope`. Raises `InitError` if the target
...
def copy_embedding_model() -> StepResult:
    ...
    embedding_model_target_dir()` (`$KLORB_DATA_DIR/embedding-model`). Raises `InitError` if
...
def copy_resource_data() -> StepResult:
    ...
    `write_config_file`/`create_symlink`. Raises `InitError` if the target can't be
    created or written.
    """
...
def run_init(scope: InitScope, *, force: bool) -> list[str]:
    """Run every `klorb init` step for `scope`, stopping at the first one that raises `InitError`
