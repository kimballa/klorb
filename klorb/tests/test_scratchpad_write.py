...
"""Tests for klorb.tools.scratchpad_write."""
...
from klorb.tools.scratchpad_write import ScratchpadWriteTool
...
def test_replaces_a_line_range(tmp_path: Path) -> None:
    ...
    result = ScratchpadWriteTool(_context(str(scratchpad))).apply({
...
def test_insert_via_single_line_replace(tmp_path: Path) -> None:
    ...
    ScratchpadWriteTool(_context(str(scratchpad))).apply({
...
def test_drift_verification_failure_names_scratchpad_reread_hint(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="re-ScratchpadRead your scratchpad"):
        ScratchpadWriteTool(_context(
...
def test_requires_active_session() -> None:
    ...
    with pytest.raises(ValueError, match="require an active session"):
        ScratchpadWriteTool(context).apply({
...
def test_name_and_parameters(tmp_path: Path) -> None:
    scratchpad = _write(tmp_path, "a\n")
    tool = ScratchpadWriteTool(_context(str(scratchpad)))
    parameters = tool.parameters()

    assert tool.name() == "ScratchpadWrite"
...
def test_summary_reports_a_line_diff(tmp_path: Path) -> None:
    ...
    assert tool.summary(args, result) == "Write scratchpad (+3/-2)"


def test_summary_on_failure_includes_the_error_and_diff_count(tmp_path: Path) -> None:
    tool = ScratchpadWriteTool(_context(str(tmp_path / "SCRATCHPAD.md")))
    args = {"start_line": 1, "end_line": 1, "start_text": "a", "end_text": "a", "new_text": "b"}

    assert tool.summary(args, error="not found") == "Write scratchpad (+1/-1) failed: not found"
