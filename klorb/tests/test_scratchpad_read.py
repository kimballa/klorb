...
"""Tests for klorb.tools.scratchpad_read."""
...
from klorb.tools.scratchpad_read import ScratchpadReadTool
...
def test_reads_whole_short_scratchpad_by_default(tmp_path: Path) -> None:
    ...
    result = ScratchpadReadTool(_context(str(scratchpad))).apply({})
...
def test_start_line_zero_means_start_at_beginning(tmp_path: Path) -> None:
    ...
    result = ScratchpadReadTool(_context(str(scratchpad))).apply({"start_line": 0})
...
def test_explicit_range_returns_requested_lines(tmp_path: Path) -> None:
    ...
    result = ScratchpadReadTool(_context(str(scratchpad))).apply({"start_line": 5, "end_line": 16})
...
def test_request_beyond_max_lines_is_capped(tmp_path: Path) -> None:
    ...
    result = ScratchpadReadTool(_context(str(scratchpad))).apply({"start_line": 1, "end_line": 500})
...
def test_negative_start_line_raises(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="start_line must be >= 0"):
        ScratchpadReadTool(_context(str(scratchpad))).apply({"start_line": -1})
...
def test_end_line_before_start_line_raises(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="end_line .* must be >= start_line"):
        ScratchpadReadTool(_context(str(scratchpad))).apply({"start_line": 10, "end_line": 5})


def test_name_and_parameters(tmp_path: Path) -> None:
    scratchpad = _write_lines(tmp_path, 1)
    tool = ScratchpadReadTool(_context(str(scratchpad)))

    assert tool.name() == "ScratchpadRead"
...
def test_requires_active_session() -> None:
    ...
    with pytest.raises(ValueError, match="require an active session"):
        ScratchpadReadTool(context).apply({})
...
def test_summary_on_failure_includes_the_error() -> None:
    tool = ScratchpadReadTool(_context(str(Path("/tmp") / "SCRATCHPAD.md")))
