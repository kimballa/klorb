...
def test_format_model_info_shows_provider_specific_capability_keys() -> None:
    ...
    assert "custom_flag: Yes" in format_model_info(model)


def test_format_model_info_includes_pricing() -> None:
    model = _model({
        "name": "some/model",
        "pricing": {"input_cost_per_mtok": 0.11, "output_cost_per_mtok": 0.8},
    })

    assert "Cost per MTok (in / out): 0.11 / 0.8 USD" in format_model_info(model)


def test_model_info_screen_shows_header_and_formatted_body() -> None:
    model = _model({"name": "some/model"})
    screen = ModelInfoScreen(model)

    container = next(iter(screen.compose()))
    header, body = container._pending_children

    assert str(header.render()) == MODEL_INFO_HEADER_TEXT
    assert str(body.render()) == format_model_info(model)


def test_show_model_info_screen_pushes_modal_for_the_active_model() -> None:
    mock_screen = MagicMock()
    model = _model({"name": "some/model"})
    mock_screen.app.get_active_model.return_value = model
    provider = ModelInfoCommandProvider(mock_screen)

    provider._show_model_info_screen()
...
def test_show_model_info_screen_reports_a_notice_when_no_model_is_active() -> None:
    mock_screen = MagicMock()
