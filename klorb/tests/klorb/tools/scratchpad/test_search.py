...
def _context(
    scratchpad_path: str, make_session_config: Callable[..., SessionConfig], *, context_lines: int = 2
) -> ToolSetupContext:
...
def test_finds_case_insensitive_match(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    scratchpad = _write(tmp_path, "one\nTWO\nthree\nfour\nfive\n")

    result = SearchScratchpadTool(_context(str(scratchpad), make_session_config)).apply({"queries": ["two"]})
...
def test_multiple_queries_combine_via_alternation(
    ...
    result = SearchScratchpadTool(_context(str(scratchpad), make_session_config)).apply({"queries": ["alpha",
        "delta"]})

    assert result["match_count"] == 2


def test_context_lines_from_config(tmp_path: Path, make_session_config: Callable[..., SessionConfig]) -> None:
    scratchpad = _write(tmp_path, "\n".join(f"line{i}" for i in range(10)) + "\n")

    result = SearchScratchpadTool(_context(str(scratchpad), make_session_config, context_lines=1)).apply(
...
def test_distant_matches_flatten_with_a_line_number_gap(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    lines = ["x"] * 20
    lines[0] = "MATCH"
...
def test_no_matches_returns_empty_lines(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    scratchpad = _write(tmp_path, "a\nb\nc\n")

    result = SearchScratchpadTool(_context(str(scratchpad), make_session_config)).apply({"queries": ["nope"]})

    assert result["match_count"] == 0
    assert result["lines"] == []


def test_query_with_regex_metacharacters_is_treated_literally(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    ...
    result = SearchScratchpadTool(_context(str(scratchpad), make_session_config)).apply({"queries": ["a(b"]})
...
def test_empty_queries_raises(tmp_path: Path, make_session_config: Callable[..., SessionConfig]) -> None:
    scratchpad = _write(tmp_path, "a\n")
...
def test_name_and_parameters(tmp_path: Path, make_session_config: Callable[..., SessionConfig]) -> None:
    scratchpad = _write(tmp_path, "a\n")
    tool = SearchScratchpadTool(_context(str(scratchpad), make_session_config))
...
def test_summary_on_failure_includes_the_error(make_session_config: Callable[..., SessionConfig]) -> None:
    tool = SearchScratchpadTool(_context(str(Path("/tmp") / "SCRATCHPAD.md"), make_session_config))

    assert tool.summary({"queries": ["x"]}, error="boom") == "Search scratchpad: ['x'] failed: boom"


def test_detail_view_caps_lines_to_60(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    lines: list[str] = []
    for i in range(70):
    ...
    scratchpad = _write(tmp_path, "\n".join(lines) + "\n")
    tool = SearchScratchpadTool(_context(str(scratchpad), make_session_config, context_lines=0))
...
def test_default_output_style_is_matches(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
...
def test_list_files_rejected_for_scratchpad(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    scratchpad = _write(tmp_path, "a\nb\n")
    with pytest.raises(ValueError, match="ListFiles.*not supported"):
        SearchScratchpadTool(_context(str(scratchpad), make_session_config)).apply(
...
def test_output_style_empty_string_defaults_to_matches(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    scratchpad = _write(tmp_path, "a\nb\nMATCH\nc\nd\n")
    result = SearchScratchpadTool(_context(str(scratchpad), make_session_config, context_lines=1)).apply(
