...
pattern matching and deny/ask/allow precedence."""
...
def test_star_matches_exactly_one_token_even_in_trailing_position() -> None:
    """`*` always means exactly one token, regardless of position."""
    table = _table(allow=[["foo", "*"]])
...
def test_trailing_question_mark_matches_zero_or_one_token() -> None:
    """`?` means zero-or-one uniformly, regardless of position."""
    table = _table(allow=[["foo", "?"]])
    assert table.evaluate(["foo"]) == "allow"
...
# fails here too, not just a matcher bug.
...
# the value half of a `--flag=value`/`key=value` argument while keeping the flag name literal.
# Only a token's own *last* character carries wildcard meaning.
...
def test_suffix_wildcard_token_matches_anything_after_literal_prefix() -> None:
    ...
    token there must start with the literal `of=` prefix."""
    pattern = ["dd", "of=*"]
    _admits(
        pattern,
        ["dd", "of=/tmp/x"],
...
def test_star_not_in_trailing_position_is_a_literal_character() -> None:
    """Only a token's own trailing `*` is a wildcard."""
...
def test_suffix_wildcard_matches_shlex_parsed_quoted_argument_forms() -> None:
    ...
    `klorb.permissions.shell_parse`'s tokenizer, which folds a quoted value
...
def test_token_matches_literal_plain_equality_when_no_trailing_star_present() -> None:
    ...
    unaffected by suffix-wildcard support."""
...
def test_double_star_before_star_also_requires_at_least_one_arg() -> None:
    """`["cmd","**","*"]` must behave identically on arity:
...
def test_double_star_backtracks_across_a_repeated_literal() -> None:
    """A greedy `**` must give tokens back so a later required literal can still land."""
    pattern = ["git", "**", "log", "**"]
    _admits(
        pattern,
        ["git", "log"],                       # both ** empty
