...
def test_search_workspace_index_gpu_enabled_defaults_true(tmp_path: Path) -> None:
    process_config = load_process_config(cwd=tmp_path, workspace=_trusted_workspace(tmp_path))
    assert process_config.session.search_workspace_index_gpu_enabled is True


def test_search_workspace_index_gpu_enabled_is_overridable_via_config_file(tmp_path: Path) -> None:
    _write_config(
        tmp_path / ".klorb" / "klorb-config.json",
    ...
    process_config = load_process_config(cwd=tmp_path, workspace=_trusted_workspace(tmp_path))
    assert process_config.session.search_workspace_index_gpu_enabled is False
...
def test_skill_rules_concatenate_across_layers(tmp_path: Path) -> None:
    ...
    assert process_config.session.skill_rules.deny == [("user", "x")]
...
def test_skill_rules_skip_malformed_entries(tmp_path: Path) -> None:
    ...
    _write_config(
        tmp_path / "etc" / "klorb-config.json",
        {"sessionDefaults": {"skillRules": {"allow": ["internal:ok", "bogus-no-colon"]}}})

    process_config = load_process_config(cwd=tmp_path)
    assert process_config.session.skill_rules.allow == [("internal", "ok")]


def test_compatibility_claude_skills_maps_from_config_key(tmp_path: Path) -> None:
    _write_config(tmp_path / "etc" / "klorb-config.json", {"compatibility.claudeSkills": True})
    process_config = load_process_config(cwd=tmp_path)
    assert process_config.compatibility_claude_skills is True
...
# --- ui.sidebar (sidebar persistence) ---


def test_persist_sidebar_creates_a_missing_file(tmp_path: Path) -> None:
    path = tmp_path / "klorb-config.json"

    process_config_module.persist_sidebar("tasks", path)

    written = json.loads(path.read_text(encoding="utf-8"))
    assert written["ui.sidebar"] == "tasks"
    assert written["schema"] == {"name": "klorb-config", "version": "1.0.0"}


def test_persist_sidebar_preserves_other_keys(tmp_path: Path) -> None:
    path = tmp_path / "klorb-config.json"
    _write_config(path, {
        "ui.theme": "monokai",
        "shell.command": "/bin/zsh",
...
def test_persist_sidebar_is_picked_up_by_a_later_load(tmp_path: Path) -> None:
    path = tmp_path / ".klorb" / "klorb-config.json"
    process_config_module.persist_sidebar("tasks", path)

    process_config = load_process_config(cwd=tmp_path, workspace=_trusted_workspace(tmp_path))

    assert process_config.sidebar == "tasks"


def test_persist_sidebar_can_toggle_back_to_none(tmp_path: Path) -> None:
    path = tmp_path / "klorb-config.json"
    process_config_module.persist_sidebar("tasks", path)
    process_config_module.persist_sidebar(None, path)

    written = json.loads(path.read_text(encoding="utf-8"))
    assert written["ui.sidebar"] is None


def test_sidebar_defaults_none(tmp_path: Path) -> None:
    process_config = load_process_config(cwd=tmp_path)
    assert process_config.sidebar is None


def test_sidebar_maps_from_config_key(tmp_path: Path) -> None:
