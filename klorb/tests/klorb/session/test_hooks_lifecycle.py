...
def test_fire_session_start_hook_runs_onsessionstart_for_a_root_session(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    marker = tmp_path / "marker"
    process_config = _process_config(tmp_path, make_session_config,
        {"onSessionStart": [_marker_handler(marker)]})
...
def test_fire_session_start_hook_is_a_noop_for_a_subagent(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    marker = tmp_path / "marker"
    process_config = _process_config(tmp_path, make_session_config,
...
def test_fire_session_start_hook_is_a_noop_without_process_config(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
...
def test_close_runs_onsessionend_for_a_root_session(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    marker = tmp_path / "marker"
    process_config = _process_config(tmp_path, make_session_config,
        {"onSessionEnd": [_marker_handler(marker)]})
    session = Session(process_config.session, process_config=process_config)
    session.close()
    assert marker.exists()


def test_close_does_not_run_onsessionend_for_a_subagent(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    marker = tmp_path / "marker"
    process_config = _process_config(tmp_path, make_session_config,
...
def test_fire_session_start_hook_carries_workspace_trust_fields(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    ...
    handler = HookConfig(
        ...
        shell=(
            ...
            'echo \'{}\''))
    process_config = _process_config(tmp_path, make_session_config, {"onSessionStart": [handler]})
    session = Session(process_config.session, process_config=process_config)
    try:
...
def test_close_does_not_carry_the_resolved_config(
    tmp_path: Path, make_session_config: Callable[..., SessionConfig]
) -> None:
    output_path = tmp_path / "output.json"
    handler = HookConfig(
        ...
        shell=(
            ...
            f'open("{output_path}", "w").write(json.dumps(data))\'; '
            'echo \'{}\''))
    process_config = _process_config(tmp_path, make_session_config, {"onSessionEnd": [handler]})
    session = Session(process_config.session, process_config=process_config)
    session.close()
    data = json.loads(output_path.read_text())
