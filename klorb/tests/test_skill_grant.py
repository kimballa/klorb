...
from klorb.permissions.skill_grant import _apply_decision_to_table, apply_skill_permission_grant
...
def test_apply_decision_allow_appends_and_removes_ask() -> None:
    ...
    result = _apply_decision_to_table(rules, ("internal", "s"), "allow")
...
def test_apply_decision_deny_writes_deny_leaves_allow() -> None:
    ...
    result = _apply_decision_to_table(rules, ("internal", "s"), "deny")
...
def test_apply_decision_never_mutates_input() -> None:
    rules = SkillRules(ask=[("internal", "s")])
    _apply_decision_to_table(rules, ("internal", "s"), "allow")
