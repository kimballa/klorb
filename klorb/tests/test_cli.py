...
# ---------------------------------------------------------------------------
# Tests for the repeated-SIGINT force-abort handler
# ---------------------------------------------------------------------------


class TestSigintHandler:
    """Verify the behaviour of `cli._sigint_handler`."""

    def test_single_sigint_raises_keyboard_interrupt(self) -> None:
        """A single ^C should raise KeyboardInterrupt via the default handler."""
        import signal

        # Reset the handler's state so the test starts clean.
        cli._last_sigint = 0.0

        # Simulate a SIGINT at a time far enough from the next one.
        fake_frame = None  # The handler only needs the signature to match.
        with pytest.raises(KeyboardInterrupt):
            cli._sigint_handler(signal.SIGINT, fake_frame)

    def test_double_sigint_calls_os_exit(self, capsys: pytest.CaptureFixture[str]) -> None:
        """Two ^Cs within the repeat window should call `os._exit(1)`."""
        import signal
        import time

        cli._last_sigint = 0.0
        # Record the current monotonic time and set _last_sigint to just now,
        # so the *next* call within _REPEAT_WINDOW triggers the abort path.
        cli._last_sigint = time.monotonic()

        mock_exit = patch("klorb.cli.os._exit", side_effect=SystemExit(1)).start()
        try:
            with pytest.raises(SystemExit):
                cli._sigint_handler(signal.SIGINT, None)

            mock_exit.assert_called_once_with(1)
            # A diagnostic message should appear on stderr.
            assert "forced exit" in capsys.readouterr().err
        finally:
            patch.stopall()

    def test_sigint_resets_after_window(self) -> None:
        """After `_REPEAT_WINDOW` elapses a second ^C should just raise
        KeyboardInterrupt again, not force-exit."""
        import signal
        import time

        # Simulate a SIGINT that happened more than _REPEAT_WINDOW ago.
        cli._last_sigint = time.monotonic() - (cli._REPEAT_WINDOW + 1.0)
