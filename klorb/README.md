...
make evals EVAL_ARGS='--suite file-tools'         # run just one suite
make evals EVAL_ARGS='--list-suites'              # print known suite names and exit
make evals EVAL_ARGS='--model openai/gpt-5-nano'  # against a specific model

# Run the risk classifier tests with the risk classifier model:
make evals EVAL_ARGS='--model openai/gpt-oss-120b:nitro --self-review --suite risk-classifier'
```
...
run against `klorb.openrouter.DEFAULT_MODEL`; `--model`/`EVAL_ARGS` can point at a different
