...
* Question: `EditFile`/`ReplaceAll`/`CreateFile` are hard-confined to `workspace_root` via
  ...
  `docs/adrs/00025-confine-file-tools-to-workspace-root.md` and
  `docs/adrs/00027-evaluate-permission-categories-deny-then-ask-then-allow.md`). Should `ReadFile`
...
* Reasoning: This closes the self-granted-read-access risk for every workspace klorb can
  ...
  by the invariant in `docs/adrs/00027-evaluate-permission-categories-deny-then-ask-then-allow.md`.
