...
* Question: `FileAccessTable` (`klorb.permissions.file_access`) now matches a `readFiles`/
  ...
  docs/adrs/file-rule-wildcard-star-matches-any-characters-including-slash.md), not just exact
...
* Reasoning: Expanding a wildcard against the sandbox's bound directories would mean walking
  ...
  good trade. `docs/adrs/bubblewrap-is-defense-in-depth-not-a-classifier-substitute.md` already
