...
* Question: `resolve_and_evaluate_read()`'s trusted-workspace branch (see
  docs/adrs/00028-gate-read-hard-boundary-on-workspace-trust.md) evaluates a candidate path against
...
* Reasoning: `"deny"` as the fallback meant a trusted project could still never prompt about a
  ...
  docs/adrs/00028-gate-read-hard-boundary-on-workspace-trust.md's own stated premise that read is the
  ...
  docs/adrs/00062-default-permission-framework-to-deny-headlessly.md), and a `readDirs.deny` entry —
