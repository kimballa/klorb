...
* Question: `PermissionAskItem.command_text` is the full, unparsed raw command a `BashTool` call
  ...
  one call (see docs/adrs/00075-permission-ask-item-carries-raw-command-text-as-its-own-field.md).
...
* Reasoning: `command_text` and `item_command_text` answer two different questions, and
  ...
  docs/adrs/00068-ask-independent-items-serially-not-just-the-strictest.md). `resource_description`
