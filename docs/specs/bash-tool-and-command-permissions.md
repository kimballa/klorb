...
`&`, a heredoc/pipe feeding an interpreter other than `cat`/`less`/`git` — escalates to `"ask"`,
never silently to `"allow"`. See docs/plans/ready/004-bash-permissions-and-bash-tool.md for the
full design and the ADRs listed at the end of this doc for the reasoning behind its most
...
* `redirects: list[RedirectTarget]` — every redirection's file target (`target`, `direction`:
  ...
  command is checked against the safe-stdin-consumer allowlist (see below).
* `forced_ask_reasons: list[str]` — every reason the walker itself escalated to `"ask"`:
  ...
  * A pipe, heredoc, or herestring feeding a command other than `cat`/`less`/`git` (the safe
    stdin-consumer allowlist — anything else that treats stdin as code to execute, `sh`/`bash`/
...
### Combining verdicts (`klorb.tools.bash.BashTool._evaluate`)

`BashTool.apply()` evaluates every contributor independently and takes the strictest
(`klorb.permissions.table.stricter_verdict`, shared with
`klorb.permissions.workspace.evaluate_write`'s own read/write merge):
...
The resulting verdict is enforced through the same `raise_if_not_allowed()` seam every other
tool uses. When the strictest contributor came from a redirection target, that target's resolved
`Path` and read/write direction are threaded onto the resulting `PermissionAskRequired`/
`PermissionError`, so a real directory-access `"ask"` still flows through `Session`'s existing
interactive-grant machinery exactly like `EditFile`'s does. A bare command-pattern `"ask"` (no
filesystem resource involved) carries no `path` — per `Session._run_tool_calls`, an ask with
`path=None` always fails closed today, regardless of `permission_framework`; a command-rule-
specific interactive grant flow (mirroring `klorb.permissions.grant` for directories) is future
work, not built here.
...
inherits the klorb process's full environment): `HOME`/`USER` always, then every
`SessionConfig.share_env` name (on-disk `shareEnv`, concatenated across layers) that's actually
set in the klorb process's own environment, then `SessionConfig.set_env` (on-disk `setEnv`,
merged key-by-key, later layers replacing earlier ones for the same key) as overrides applied
last.
...
* **Negative**, `-signum` — Python's ordinary direct-child-signaled convention. This happens when
  ...
  positive code — confirmed empirically (see `_decode_exit`'s docstring), which is *more* cases
  than docs/plans/ready/004-bash-permissions-and-bash-tool.md's own "process outcome" section
  anticipated (it only described the positive-code shape).

Any other non-zero, non-signal exit reports `"Process completed normally with non-zero status"`.

### Response shape

`BashTool`'s result dict uses this codebase's ordinary snake_case tool-response convention (not
the plan doc's illustrative camelCase sketch): `command`, `exit_status`, `success`,
...
mount/namespace/env argv described in the plan's "bubblewrap args to use" section — is a stub
that raises `NotImplementedError` and is never called: developing and verifying it requires a
...
session, the result carries a `sandbox_notice` string (never repeated for the rest of that
session — tracked by `id(session_config)`, which naturally resets on `/clear` since a fresh
`SessionConfig` is created then) explaining why (missing `bwrap` binary, this environment
disallowing unprivileged user namespaces, or `bwrap` being available but klorb's sandboxed
execution simply not being built yet) and noting that command/redirect permission checks are
still fully enforced regardless.
...
* A command-rule-specific interactive "ask" grant flow (mirroring
  `klorb.permissions.grant`/`PermissionAskScreen` for directories) — a bare command-pattern
  `"ask"` fails closed today regardless of `permission_framework`, since it carries no `path` for
  `Session`'s existing grant machinery to act on.
...
* Structured audit logging of command requests/decisions/outcomes (goal #7 in the plan doc's
  "Context" section) — real goal, not required for this first version.
* macOS support (`sandbox-exec`/Seatbelt in place of `bwrap`) — Linux only.

## See also

* docs/plans/ready/004-bash-permissions-and-bash-tool.md — the full design this spec summarizes.
